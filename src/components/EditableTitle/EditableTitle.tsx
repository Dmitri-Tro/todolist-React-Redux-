...
export const EditableTitle: FC<EditableTitleProps> = React.memo(({oldTitle, setNewTitle}) => {
            ...
            <>
                {mode === 'viewMode' ?
                    <span className={styles.editableTitle}
                          onDoubleClick={() => setMode('inputMode')}>{oldTitle}</span> :
                    <input className={styles.editableTitleInput} value={updatedTitle} onChange={onTitleInputChangeHandler}
                           onBlur={onInputBlurHandler} autoFocus/>
                }
            </>
        )
    });