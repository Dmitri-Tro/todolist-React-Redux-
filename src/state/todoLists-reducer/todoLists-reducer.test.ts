import { todoListsReducer, removeTodoListAC, addTodoListAC, updateTodoListTitleAC } from "./todoLists-reducer";
import { TodoListDomain } from "types/types";

let startData: TodoListDomain[];
beforeEach(() => {
    startData = [
...
test("Reducer should add todoList", () => {
    const endData = todoListsReducer(
        startData,
        addTodoListAC({ id: "3", title: "New TodoList", addedDate: new Date(), order: 0, filter: "All" }),
    );
